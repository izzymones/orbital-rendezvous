...
class CWModel:
    def __init__(self, params):
        ...
        vx_dot = 2*self.p.n*v[1] + 3*self.p.n**2*r[0] + self.u[0]
        vy_dot = -2*self.p.n*v[0] + self.u[1]
        vz_dot = -self.p.n**2*r[2] + self.u[2]
        ...
        x_r = ca.DM(self.p.xr).reshape((6,1))
        x_error = self.state - x_r
        x_cost = x_error.T @ self.p.Q @ x_error 
        terminal_cost = x_error.T @ (self.p.terminal_cost_factor * self.p.Q) @ x_error 
        u_goal = ca.DM(self.p.ur).reshape((3,1))
        u_error = self.u - u_goal
        u_cost = u_error.T @ self.p.R @ u_error
        cost = x_cost + u_cost
